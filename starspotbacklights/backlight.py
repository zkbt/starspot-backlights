...
try:
    ...
    warnings.warn(
        """
    `emcee` or `corner` not found.
    You won't be able to sample,
    but you can still use the
    rest of the modeling tools!
    """
    )
...
# what is the maximum allowed covering spot fraction f
maxf = 0.5
...
def guesstimate_deltaf(f, f1=0.001):
    """
    Make a Poisson-based estimate of the time-variable
    fraction of the stellar surface covered by spots,
    given some total covering fraction and the size
    of a typical spot.
...
class Backlight:
    ...
    """
    An object representing a star with some hot/cold spots on
    it, which with some parameters can produce various
    ...
    def set_parameters(self, parameters):
        ...
        ----------
        parameters : list
            The list of numerical values for parameters, in
            the following order:
                T_unspot = temperature of unspotted surface (K)
                T_spot = temperature of spotted surface (K)
                f = total spot-covering fraction
                deltaf = time-variable component of spot-covering fraction
                f1 = size of a single spot (as fraction of star)
                rprs = the actual planet-to-star radius ratios
        """
        if self.offset_kludge:
            (
                self.T_unspot,
    ...
    def plot_everything(self, N_lines=20, factor=1.25, seed=42, **kw):
        ...
        gs = plt.matplotlib.gridspec.GridSpec(
            2, 2, width_ratios=[2, 1], wspace=0.35, bottom=0.15, top=0.95, right=0.9
        )
